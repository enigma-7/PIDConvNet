...
for l in range(1,7):
    ...
    X_[str(l)]  = np.append(dataset[:,0][mask].reshape(-1,l), dataset[:,6][mask].reshape(-1,l), axis=1)
...
for l in range(1,7):
    ...
    history = model.fit(X[:,:l], T, batch_size=batch[l-1], epochs=epoch[l-1], validation_data=(Xv[:,:l],Tv), )
    P1 = model.predict(Xt[:,:l]).reshape(-1)
    P1_[str(l)] = P1
    ...
    pioncon2[l-1]  =  FPR[TPR<0.905][0]
    Upioncon2[l-1] = uFPR[TPR<0.905][0]

plt.figure(figsize=(8,6))
plt.errorbar(range(1,7), pioncon1, Upioncon1, 0, 'k.', label = "ANN-V")
plt.errorbar(range(1,7), pioncon2, Upioncon2, 0, 'b.', label = "ANN_S")
plt.legend()
plt.xlabel("Layers")
