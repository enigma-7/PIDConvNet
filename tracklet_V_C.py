...
dataset, infoset = DATA.ocdb_tracklet_(dataset, infoset, DEFAULTS.ocdbdir)
...
cp, b, p = axes[1].hist(p_pred, color = 'r', label=cnames[0], bins = b, histtype='step', linewidth=2.0)
ce, b, p = axes[1].hist(e_pred, color = 'g', label=cnames[1], bins = b, histtype='step', linewidth=2.0)
axes[0].set_yscale(scale)
axes[0].vlines(thresholds[TPR<0.9][0], 0, max(c1), 'k', label="Decision Boundary")
...
cp, b, p = axes[1].hist(p_pred_track, color = 'r', label=cnames[0], bins = b, histtype='step', linewidth=2.0)
ce, b, p = axes[1].hist(e_pred_track, color = 'g', label=cnames[1], bins = b, histtype='step', linewidth=2.0)
